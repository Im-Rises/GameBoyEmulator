...
void Ppu::draw(const int& cycles)//Not working
{
	...
	{
		drawLineSimulation();
		drawBackgroundLine();//Crash when uncommented
	}
...
void Ppu::drawBackgroundLine()
{
	...
	if (testBit(lcdc, 0))
	{
		...
		uint8_t tileRow = (yPos / 8) * 32;

		for (int pixel = 0; pixel < DOTS_DISPLAY_X; pixel++)
		{
			uint16_t xPos = pixel + scx;
			...
			if (unsignedValue)
				tileNum = (uint8_t)memory->read(backgroundMemory + tileRow + tileColumn);
			else
				tileNum = (int8_t)memory->read(backgroundMemory+ tileRow + tileColumn);

			if (tileNum > 0)
				cout << "Fonctionne" << endl;//ERROR HERE tileNum is completely wrong !
